*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZV_V4_MSRV
*   generation date: 28.02.2025 at 19:06:47
*   view maintenance generator version: #001407#
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZV_V4_MSRV          .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZV_V4_MSRV'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZV_V4_MSRV_EXTRACT
TOTAL = ZV_V4_MSRV_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZV_V4_MSRV'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZV_V4_MSRV-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZV_V4_MSRV_EXTRACT
TOTAL = ZV_V4_MSRV_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZV_V4_MSRV-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZV_V4_MSRV'
...
TOTAL = ZV_V4_MSRV_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZV_V4_MSRV-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZV_V4_MSRV-UPD_FLAG NE SPACE.
